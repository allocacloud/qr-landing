...
module.exports = (app) => {
    return (req, res, next) => {
        ...
        res.ts = require('./lang/' + res.locals.lang + '.json')
        ...
        app.t = res.locals.t = str => res.ts && res.ts.hasOwnProperty(str) ? res.ts[str] : str
        ...
        if (app.config.logos && app.config.logos[req.params.id]) {
            res.locals.logo = app.config.logos[req.params.id]
        }
